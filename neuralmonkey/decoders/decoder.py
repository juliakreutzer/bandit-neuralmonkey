...
class Decoder(ModelPart):
    ...
    def __init__(self,
        ...
        with tf.variable_scope(name):
            ...
            self.embedded_go_symbols = tf.nn.embedding_lookup(self.embedding_matrix,
                                                     self.go_symbols)
            ...
            for i, temp in zip(range(self.max_output_len+1), temps):
                ...
                if prev is None:
                    ...
                    with tf.variable_scope("loop_function", reuse=True):
                        ...
                        sample_logprob_word = tf.dynamic_partition(out_activation, partitions=partitions, num_partitions=2)[1]

                        # compute log probabilities from logits
                        z = tf.reduce_logsumexp(out_activation, [1])  # batch_size vector
